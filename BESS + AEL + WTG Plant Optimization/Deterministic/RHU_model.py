...
#%% RHU model
def RHU_model(RHU_Parameters, P_gen, Price_El, Price_H2):
    # Unpack parameters
    ...
    if not RHU_Parameters['Electricity Market']: # Electricity market participation is disabled
        def e_market_disabled_rule_1(model, t):  #
            return model.P_WTG_Grid[t] ==  0
        model.e_market_disabled_rule_1 = pyo.Constraint(model.time, rule=e_market_disabled_rule_1)

    if not RHU_Parameters['AEL Pruchases']: # Electricity market participation is disabled
        def AEL_pruchases_disabled_rule_1(model, t):
            return model.P_Grid_AEL[t] ==  0
        model.AEL_pruchases_disabled_rule_1 = pyo.Constraint(model.time, rule=AEL_pruchases_disabled_rule_1)
    # Objective function
