...
def run_simulation(start, end, RHU_Parameters_Global, H2_price, dataset, sim_name):
    ...
    while day != pd.Timestamp(end) + pd.Timedelta('1d'):
        ...
        Sim_Results['Generated H2'].append(H_day)
        Sim_Results['Energy market accumulated benefits'] += CF_day_arb
        Sim_Results['Hydrogen production benefits'] += CF_day_H2
        # Emptying stored hydrogen
        emptying_counter += 1
        if emptying_counter == RHU_Parameters['Tank emptying']:
            RHU_Parameters['Initial hydrogen level'] = 0
            emptying_counter = 0
        else:
            RHU_Parameters['Initial hydrogen level']+= H_day
    ...
    AEL_H2_gen = sum(Sim_Results['Generated H2'])
